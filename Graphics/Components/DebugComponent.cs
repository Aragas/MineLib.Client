...
namespace MineLib.PCL.Graphics.Components
{
    public sealed class DebugComponent<T> : DrawableGameComponent where T : struct, IVertexType
    {
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            var gpu = string.Format("Chunks : {0}",                 WorldComponent<T>.Chunks);
            var cpu = string.Format("Build Chunks : {0}",           WorldComponent<T>.BuildedChunks);
            var opa = string.Format("Opaque Sections : {0}",        WorldComponent<T>.DrawingOpaqueSections);
            var tra = string.Format("Transparent Sections : {0}",   WorldComponent<T>.DrawingTransparentSections);
